...
class Tetromino:
   ...
   def __init__(self, type,grid_height, grid_width):
      ...
      if type == 'I':
         ...
         self.occupied_tiles.append((0, 1)) # (column_index, row_index)
         self.occupied_tiles.append((1, 0))
         self.occupied_tiles.append((1, 1))
   ...
   # A function that turns tetromino 90 degree counterclokwise
   def reverse_rotate(self,game_grid,current_tetromino):
      controller=self.rotate_checker_y(current_tetromino, game_grid)
      if controller is True:
         n = len(self.tile_matrix)
         for x in range(0, int(n / 2)):

            for y in range(x, n - x - 1):
               temp = self.tile_matrix[x][y]
               self.tile_matrix[x][y] = self.tile_matrix[y][n - 1 - x]

               self.tile_matrix[y][n - 1 - x] = self.tile_matrix[n - 1 - x][n - 1 - y]

               self.tile_matrix[n - 1 - x][n - 1 - y] = self.tile_matrix[n - 1 - y][x]

               self.tile_matrix[n - 1 - y][x] = temp
         # Use the rotate_checker function to prevent the tetromino from going out of game grid
         self.rotate_checker(current_tetromino, game_grid)

   # A function that turns tetromino 90 degree clokwise
   def rotate(self,grid,current_tetromino):
      # Calling reverse_rotate function 3 times means rotating tetromino 90 degree clockwise
      self.reverse_rotate(grid, current_tetromino)
      self.reverse_rotate(grid, current_tetromino)
      self.reverse_rotate(grid, current_tetromino)

   # A function that makes rotate work properly
   def rotate_checker(self,current_tetromino,game_grid):
      n = len(self.tile_matrix)  # n = number of rows = number of columns
      for row in range(n):
         for col in range(n):
            if self.tile_matrix[row][n-1-col] is not None:
               rightmost = self.get_cell_position(row, n - 1 - col)
                  # tetromino cannot go left if any leftmost tile is at x = 0
               if rightmost.x == self.grid_width and n==3:
                  current_tetromino.move("left",game_grid)
               elif rightmost.x == self.grid_width and n==4:
                  current_tetromino.move("left",game_grid)
                  current_tetromino.move("left",game_grid)
                  current_tetromino.move("left", game_grid)
            if self.tile_matrix[row][col] is not None:
   ...
   # A function that checks y position of tetrominos, if is near to bottom dont rotate
   def rotate_checker_y(self,current_tetromino,game_grid):
      n = len(self.tile_matrix) # n = number of rows = number of columns
      for col in range(n):
         for row in range(n - 1, -1, -1):
            if self.tile_matrix[row][col] is not None:
               bottommost = self.get_cell_position(row, col)
               if bottommost.y <= 1 and n==3:
                  return False
               if bottommost.y <= 2 and n == 4:
                  return False
               else:
                  return True
